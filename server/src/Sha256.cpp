...
// 64 primes, per the FIPS 180-4 specification.
...
std::array<uint8_t, Sha256::kDigestSize> Sha256::finalize() {
    ...
    uint8_t lenBytes[8];
    for (int i = 0; i < 8; ++i) {
        lenBytes[7 - i] = uint8_t(bitLen >> (8 * i));
    }
    // append length directly into the buffer without touching totalLen_
    // (update() would count these bytes toward the message length)
