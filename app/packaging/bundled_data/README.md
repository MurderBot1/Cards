...
backend/data/
  cards.sqlite3
  card-vectors.cvi
  yolo_card_detector.onnx
  yolo_card_detector.names.json
  dinov2_vits14.onnx
  ocr_det.onnx
  ocr_rec.onnx
  ocr_dict.txt
...
(torch, ultralytics, paddleocr — see `native/cardnet/export/`) — not
something an end user should run.
...
**Ship with it.** Copy the files above into this folder before
...
Trade-off: `cards.sqlite3` covering all three games plus the cardvec
vector index plus the four ONNX models can easily be several hundred MB
to a few GB, on top of what ONNX Runtime adds to every build (tens of
MB — far less than the ~1-2GB torch/paddleocr/paddlepaddle used to,
before `native/cardnet` replaced them). That's the real size of the installer
